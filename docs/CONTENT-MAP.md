...
The complete catalog of the workbook (`workbook/agentic-development-study.html`, v1.70):
10 modules, 66 topics, 6 research reports, 2 inline map sections, 53 deep-dive companions.
...
| S7 Contract & brownfield surfaces; the secrets problem | `sec-surfaces` | ✅ (v1.70) |
...
`S3-advisory-vs-deterministic-rules-backdoor` · `S4-mcp-tools-attack-surface` · `S5-loop-autorun-selfmod-sandbox` · `S6-memory-poisoning-multiagent-risk` · `S7-contract-brownfield-surfaces-secrets` · `PB1-assess-runbook` ·
...
`#p1-deepdive` … `#p5-deepdive`, `#d1-deepdive` … `#d7-deepdive`, `#h1-deepdive` … `#h4-deepdive`, `#i1-deepdive` … `#i7-deepdive`, `#b1-deepdive` … `#b6-deepdive`, `#s1-deepdive`, `#s2-deepdive`, `#s3-deepdive`, `#s4-deepdive`, `#s5-deepdive`, `#s6-deepdive`, `#s7-deepdive`, `#pb1-deepdive` … `#pb5-deepdive` (each appears
exactly twice: the "Go deeper" link + the JS open handler).

## Gallery

- `gallery.html` — browsable grid of all 94 diagrams from the deep-dive companions,
